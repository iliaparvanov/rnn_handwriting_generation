...
class Model(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        outputs = tf.TensorArray(tf.float32, args.T)
        ...
        for t in tf.range(args.T):
            rnn_1_out, rnn_1_h, rnn_1_c = self.rnn1(tf.concat([tf.gather(x, t), w], 2), (rnn_1_h, rnn_1_c))   
            ...
            phi = tf.reduce_sum(input_tensor=tf.exp(tf.square(-u + kappa) * (-beta)) * alpha, axis=1,
                                    keepdims=True)
            w = tf.matmul(phi, c_vec)
            w = tf.squeeze(w, 1)
            w = tf.keras.layers.Reshape((1, args.c_dimension))(w)
            rnn_1_reshaped = tf.keras.layers.Reshape((1,
                                            args.rnn_state_size))(rnn_1_out)
            rnn_2_input = tf.concat([tf.gather(x, t), rnn_1_reshaped, w], 2)
            rnn_2_out, rnn_2_h, rnn_2_c = self.rnn2(rnn_2_input, (rnn_2_h, rnn_2_c))
            outputs = outputs.write(t, rnn_2_out)
        
        output = self.linear(tf.reshape(outputs.stack(), [-1, args.rnn_state_size]))
        return output
...
class CustomLoss(tf.keras.losses.Loss):
    def call(self, y_true, y_pred):
        return compute_custom_loss(y_true, y_pred)